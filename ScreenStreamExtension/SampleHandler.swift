...
nonisolated let logger = LBLogger.with("com.kdt.livecontainer.screenstream")
...
final class SampleHandler: RPBroadcastSampleHandler, @unchecked Sendable {
    ...
    override func broadcastStarted(withSetupInfo setupInfo: [String: NSObject]?) {
        Task {
            do {
                guard let url = ScreenStreamPreference.shared.makeURL() else {
                    finishBroadcastWithError(NSError(domain: "LiveContainerScreenStream", code: 1))
                    return
                }
